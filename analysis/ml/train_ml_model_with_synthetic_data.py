...
def assign_department(row):
    if pd.notna(row.get('CLAIM_AMOUNT_PAID', 0)) and row.get('CLAIM_AMOUNT_PAID', 0) > 5000:
        return 'High Value Claims'
    
    elif pd.notna(row.get('WARRANTY', '')) and 'third-party' in str(row.get('WARRANTY', '')).lower():
        return 'Legal Claims'
    
    elif pd.notna(row.get('POLICYHOLDER_AGE', 0)) and row.get('POLICYHOLDER_AGE', 0) > 65:
        return 'Senior Claims'
    
    elif pd.notna(row.get('PREMIUM_AMOUNT_PAID', 0)) and row.get('PREMIUM_AMOUNT_PAID', 0) > 400:
        return 'VIP Claims'
...
for col in df.columns:
    ...
    elif col != 'CLAIM_DATE':  # Skip datetime column
        df[col] = df[col].fillna(df[col].median())

X_df = df.copy()

X_df = X_df.drop('CLAIM_DATE', axis=1)
...
print(f"Label encoders saved to {output_dir / 'label_encoders.joblib'}")

X = X_df.drop(['CLAIM_ID', 'DEPARTMENT'], axis=1)
y = X_df['DEPARTMENT']
...
print(f"Department distribution plot saved to {output_dir / 'department_distribution.png'}")
...
joblib.dump(model_metadata, output_dir / 'model_metadata.joblib')
print(f"Model metadata saved to {output_dir / 'model_metadata.joblib'}")
...
with open(output_dir / 'model_training_report.md', 'w') as f:
    ...
    f.write('| Department | Count | Percentage |\n')
    f.write('| ---------- | ----- | ---------- |\n')
    ...
    f.write('The ML model was trained using synthetic department assignments based on business rules with adjusted thresholds to create a diverse set of classes. In a real-world scenario, this would be replaced with actual historical routing decisions.\n\n')
