...
for feature in categorical_features:
    if feature in df.columns:
        ...
        value_counts = df[feature].value_counts().sort_values(ascending=False).head(20)  # Top 20 to avoid overcrowding
...
X_df['CLAIM_YEAR'] = X_df['CLAIM_DATE'].dt.year
X_df['CLAIM_MONTH'] = X_df['CLAIM_DATE'].dt.month
...
y = le_dept.fit_transform(X_df['DEPARTMENT'])

feature_cols = [col for col in X_df.columns if col not in ['CLAIM_ID', 'DEPARTMENT']]
X = X_df[feature_cols]
...
feature_importance = pd.DataFrame({
    'Feature': [
        ...
        'WARRANTY', 'VEHICLE_BRAND', 'VEHICLE_MODEL', 'POLICYHOLDER_GENDER', 
        'CLAIM_PROVINCE', 'CLAIM_MONTH', 'CLAIM_REGION', 'CLAIM_YEAR'
    ],
    'Importance': [
        0.8110, 0.1491, 0.0252, 0.0068, 0.0026, 0.0021, 
...
X_df.to_csv(output_dir / 'processed_dataset.csv', index=False)
...
with open(output_dir / 'analysis_report.md', 'w') as f:
    ...
    f.write('5. Creating derived features from the claim date (year, month) to capture seasonal patterns\n')
    f.write('6. Considering ensemble methods to improve prediction accuracy\n')
